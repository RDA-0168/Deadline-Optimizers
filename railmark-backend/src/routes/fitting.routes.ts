...
router.post(
  '/',
  authenticate,
  authorize([UserRoles.ADMIN, UserRoles.INSPECTOR]),
  validate(createFittingSchema),
  FittingController.createFitting
);
...
router.delete(
  ...
  authenticate,
  authorize([UserRoles.ADMIN]),
  validate(getFittingByIdSchema),
  FittingController.deleteFitting
);
...
router.post(
  '/:fittingId/lifecycle',
  optionalAuthenticate,
  validate(createLifecycleSchema),
  LifecycleController.appendLifecycle
);
