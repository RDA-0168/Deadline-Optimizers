...
function filterFallbackFittings(options: { status?: string; zone?: string; type?: string; limit?: number; offset?: number }) {
...
export class FittingService {
  ...
  static async getFittingById(id: string) {
    try {
      ...
      if (!fitting) {
        const fallback = MASTER_12_FITTINGS_FALLBACK.find(
          (f) => f.id === id || f.qrCodeValue === id || f.qrId === id
        );
        if (fallback) {
          return {
            basicInfo: fallback,
