...
export class InspectionService {
  ...
  static async getInspectionsByFittingId(fittingId: string) {
    try {
      const inspections = await prisma.inspection.findMany({
        where: { fittingId },
        orderBy: { inspectionDate: 'desc' },
  ...
  static async createInspection(data: any, inspectorName?: string) {
    ...
    const created = await prisma.inspection.create({
      data: {
        ...
        condition: condition,
        qrReadability: (data.qrReadability as any) || 'Good',
  ...
  private static formatInspectionResponse(i: any) {
    return {
      id: i.id,
