...
4. **Append-Only Custody Log**: Every maintenance event (torque check, pad replacement, lubrication) is cryptographically timestamped into the immutable audit trail.`,
    ...
    // 10. Direct Capabilities / "Can you..." Inquiries
    const isCapabilityQuery = q.startsWith('can you') ||
        q.startsWith('could you') ||
        q.startsWith('are you able to') ||
        q.startsWith('will you') ||
        q.startsWith('do you know how to') ||
        q.includes('write an essay') ||
        q.includes('write a poem') ||
        q.includes('write a story') ||
        q.includes('book a ticket') ||
        q.includes('hack') ||
        q.includes('stop the train') ||
        q.includes('delete database');
    if (isCapabilityQuery) {
        // Check if it's asking for something RailMark AI CAN do
        const isSupportedAction = q.includes('fitting') ||
            q.includes('qr') ||
            q.includes('inspect') ||
    ...
    // 11. General Unmatched Queries (Straightforward Scope Clarification)
    return {
...
export async function chatWithEdith(userPrompt, history = [], contextData) {
    ...
    const isFounderQuery = queryLower.includes('founder') ||
...
//# sourceMappingURL=edith.service.js.map